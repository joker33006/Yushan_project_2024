...
library(patchwork)
library(showtext)
w_path <- "D:/climate_data/TCCIP/日資料/1960_2023/"
...
dict <- data.table(
  vt = 1:8,
  ch_name = c("山麓帶","次山地帶下層","次山地帶上層","山地帶下層",
           "山地帶上層","上層山地帶","次高山帶","高山帶")
)
vt_stat <- vt_stat[dict,on=.(VT=vt)]
vt_stat[,ch_name:=factor(ch_name,levels=c("山麓帶","次山地帶下層",
                                        "次山地帶上層","山地帶下層",
                                        "山地帶上層","上層山地帶",
                                        "次高山帶","高山帶"))]
showtext_auto()
...
ggplot(data=vt_stat,aes(x=year,y=temp))+
  ...
  facet_wrap(ch_name~.,ncol=2,scales = "free_y")+
  geom_smooth(color="darkred")+
  scale_x_continuous(breaks = seq(1995,2023,5))+
  labs(x="Year",y="Temperature (°C)")+
  theme(text = element_text(family = '微軟正黑體',size=20))+
  theme_bw()
...
```{r}
era5_dt <- fread(paste0(pro_path,"rawdata/weather_index_data.csv"))
era5_coord <- fread(paste0(pro_path,"rawdata/ERA5_data_sample_point_yushan.csv"))
era5_dt <- era5_dt[era5_coord,on=.(plot_id)]
setnames(era5_dt,"summit","plot_id")
setnames(era5_dt,c("LON","LAT"),c("x","y"))
...
era5_monthly_reg <- 
  era5_dt[,.(spei_12=mean(spei_12,na.rm=T)),by=.(year,month)]
...
ggplot(data=era5_monthly_reg,aes(x=date,y=spei_12,fill=spei_12))+
  ...
  scale_x_date(breaks="2 years",
               ...
               date_labels = "%Y")
 ggsave(paste0(pro_path,"result/plot/spei_12_1990_2025_09.jpeg"),
        height = 5,width = 10,dpi=300)

era5_sf_ext <-st_as_sf(era5_dt[year==2021&month==5],wkt ="wkt",crs=4326) 
era5_sf_res<-st_as_sf(era5_dt[year==2024&month==5],wkt ="wkt",crs=4326)
era5_sf_fin<-st_as_sf(era5_dt[year==2025&month==5],wkt ="wkt",crs=4326)
p_date <- list(c(1991,12),c(2021,4),c(2025,8))
spei_p <- lapply(p_date,function(date_v){
  dt <-st_as_sf(era5_dt[year==date_v[1]&month==date_v[2]],wkt ="wkt",crs=4326)
  p <- ggplot()+
       geom_sf(data=dt,aes(fill=spei_12),color="gray")+
       geom_sf(data=region,fill=NA)+
       labs(fill="SPEI-12",title =paste0(date_v[1],"-",date_v[2]))+
       scale_fill_gradient2(
            high ="#00896C",
            mid = "gray95",
            low = "#CAAD5F",
   ...
   geom_text(data=dt,aes(x=x,y=y,label=round(spei_12,3)))+
  theme_bw()
  return(p)
})
(spei_p[[1]]+spei_p[[2]])/(spei_p[[3]]+ plot_spacer())+ plot_layout(guides = "collect")+ plot_annotation(tag_levels = 'A')
